...
class BisonExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self._last_trades_poll_bison_timestamp = 1.0
    ...
    def authenticator(self):
        return BisonAuth(
            bison_rpc_host=self.rpc_host,
            bison_rpc_username=self.rpc_username,
            bison_rpc_password=self.rpc_password,
            bison_cert_path=self.cert_path,
            bison_app_password=self.app_password,
    ...
    async def _place_order(self,
        ...
        type_str = BisonExchange.bison_order_type(order_type)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(bison_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/bison-exchange/bison-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Bison the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Bison's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Bison's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_bison_timestamp * 1e3)
            self._last_trades_poll_bison_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(bison_utils.is_exchange_information_valid, exchange_info["symbols"]):
